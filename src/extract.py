...
def text_to_spec(document_text):
	...
	messages = [{"sender": "user", "text": document_text}]
...
def process_text(documents_folder_name, document_text_file_name):
	...
	with open(document_text_file_path, "r") as document_text_file:
		...
		#document_text = normalize_text(document_text_file.read())
	document_extract_file_name = document_text_file_name.replace(".txt",".extracted.txt")
	...
	with open(document_extract_file_path, "w") as document_extract_file:
		...
		for i,chunk in enumerate(chunks):
			print(f"  -> extracting spec from chunk #'{i+1}/{len(chunks)}'...\n===")
			specs = text_to_spec(f"Construction Document:\n{chunk}")
			print(specs)
